...
async function uploadPhoto(file: File, labels: { failed: string; tooLarge: string; noMediaId: string }) {
...
export function OpenWhenMessagesClient() {
  ...
  function startEdit(message: OpenWhenMessage) {
    ...
    setForm({
      ...
      attachments: Array.isArray(message.attachments)
        ? message.attachments.map(getAttachmentFromValue).filter((item): item is PhotoAttachment => Boolean(item))
        : [],
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!editingId && openWhenMessageCountLimit !== null && messages.length >= openWhenMessageCountLimit) {
  ...
  return (
    <div className="h-full overflow-y-auto bg-[linear-gradient(180deg,#fafafa_0%,#f8fafc_100%)]">
      ...
      <div className="mx-auto grid w-full max-w-[1500px] gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,1fr)_420px] lg:pb-24">
        <main className="min-w-0 space-y-6">
          ...
          {reachedMessages.length > 0 ? (
            <section className="rounded-[24px] border border-amber-200 bg-amber-50 p-5 text-sm text-amber-950 shadow-[0_14px_34px_rgba(245,158,11,0.1)]">
          ...
          <section data-tour="open-when-list" className="space-y-4">
            {loading ? (
              <div className="rounded-[24px] border border-white/75 bg-white/80 p-5 text-sm text-stone-600 shadow-sm">
                {t('loading')}
              </div>
            ) : null}
            ...
            {!loading && messages.length === 0 ? (
              <div
                ...
                className="rounded-[30px] border border-dashed border-purple-200 bg-white/80 p-10 text-center shadow-sm"
              >
            ...
            {messages.map((message) => {
              ...
              return (
                <article
                  ...
                  data-tour="open-when-list-item"
                  className="rounded-[28px] border border-white/75 bg-white/88 p-5 shadow-[0_16px_45px_rgba(15,23,42,0.06)] backdrop-blur-sm sm:p-6"
                >
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex shrink-0 gap-2">
                      <button
                        ...
                        className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-stone-200 bg-white text-stone-600 transition hover:border-purple-200 hover:text-purple-700"
                      ...
                      <button
                        ...
                        className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-stone-200 bg-white text-stone-600 transition hover:border-red-200 hover:text-red-700"
        ...
        <aside className="lg:sticky lg:top-6 lg:self-start">
          <form
            ...
            className="rounded-[28px] border border-white/75 bg-white/90 p-5 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur-sm sm:p-6"
          >
            <div data-tour="open-when-form-header" className="flex items-start justify-between gap-4">
              <div>
            ...
            <div className="mt-5 space-y-4">
              ...
              <div className="grid gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="text-sm font-semibold text-stone-800">{t('form.colorIcon')}</div>
                    ...
                    {GROUP_COLOR_OPTIONS.map((option) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          />
                          <span className="block leading-none">{t(`colorLabels.${option.key}`)}</span>
                        </button>
                      )
                    })}
                  </div>
                ...
                {form.attachments.length > 0 ? (
                  <div className="grid grid-cols-4 gap-2">
                    {form.attachments.map((attachment) => (
                      <div key={attachment.id} className="group relative aspect-square overflow-hidden rounded-2xl border border-stone-200 bg-stone-50">
                        <img
                          src={attachment.url}
                          alt=""
                  ...
                  {lovedOnes.map((person) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <span className="min-w-0 flex-1 truncate">{getLovedOneLabel(person)}</span>
                      </label>
                    )
                  })}
                </div>
              ...
              <label className="block">
                <span className="text-sm font-semibold text-stone-800">{t('form.optionalDate')}</span>
                <input
                  type="date"
              ...
              <button
                ...
                className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-purple-600 px-5 py-3 text-sm font-semibold text-white shadow-[0_16px_35px_rgba(124,58,237,0.24)] transition hover:bg-purple-700 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <PlusIcon className="h-4 w-4" />
                {saving ? t('actions.saving') : editingId ? t('actions.saveChanges') : t('actions.create')}
              </button>
